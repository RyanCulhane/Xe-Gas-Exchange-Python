 ...
 *           enjoy the speed.
 *  NOTE #3: This code is actually used to compile the lib for python code
 *
...
double* sparse_gridding_distance(double *coords, double kernel_width,
...
// 	unsigned int *sparse_voxel_indices;
	...
	//Ziyi, define return array
	double *return_array = calloc(3 * max_size, sizeof(double));
	double *nonsparse_sample_indices = return_array;
	double *nonsparse_voxel_indices = &return_array[max_size];
	double *nonsparse_distances = &return_array[2*max_size];
...
*/
	...
	//combine the output together to return

	// for (p=0;p<max_size;p++){
	// 	return_array[p] = nonsparse_sample_indices[p];
	// }
	// for (p=max_size;p<2*max_size;p++){
	// 	return_array[p] = nonsparse_voxel_indices[p-max_size];
	// }
	// for (p=2*max_size;p<3*max_size;p++){
	// 	return_array[p] = nonsparse_distances[p-2*max_size];
	// }
